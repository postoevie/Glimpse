...
3. **One slice ≈ one Glimpse PR** when practical (see Grab `docs/public-project-migration.md`).
...
| **Features** | Partial **X3** (nav shell skeleton only) |
| **Exit** | Glimpse builds and shows root UI; Grab can link the three packages locally |
...
## Grab bridge (per increment)

| When | Action |
|---|---|
| After **I0** | Confirm Grab SPM paths resolve to `../Glimpse/Packages/*` |
| After each **I1+** PR in Glimpse | Rebuild Grab against local packages; no private fork of Core/AI/Features |
| Private-only work | Stays in Grab — never blocks Glimpse increment exit |
...
- [x] Grab ↔ Glimpse bridge notes per increment.
