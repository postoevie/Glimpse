...
- User may optionally choose a custom folder and/or type their own translation at capture time — never required. The **add view prefills the default custom folder** (last used or last manually chosen) when its `sourceLanguage` is empty or matches the card's pending source; user can change or clear. User may **optionally pick source and target language** when the UI allows (source: auto-detected, or manual override; if detection fails and user skips manual pick, source stays **`null`**). **Selecting a custom folder prefills `sourceLanguage` and `targetLanguage` from that folder if set** — source locked after prefill; target can still change before save. Choosing/saving with a folder updates the default. Custom folder picker shows only folders whose `sourceLanguage` is empty or matches the card's source (including `null`).
...
**Resolved — capture surfaces for v1:** Quick note widget, Share Extension, in-app add. **Widget/Share Extension:** `text`, optional `translation` — **no folder picker**; applies **default custom folder** on save when language matches, otherwise no custom folder. **In-app add:** optional source/target pickers; default custom folder **prefilled** on add view (same language gate). Clipboard-detection deferred to v2.
...
- User may additionally file an item into **at most one custom folder** — free-form name (required), flat list only (no nesting, no multi-membership). A custom folder may contain **zero items**. Its **`sourceLanguage` is set by the first item added**, then locked (even at 0 items) — only items with the same `sourceLanguage` (including `null`) can join afterward. Its **`targetLanguage` is optional** — updated when a card with a target is added, or when any card in the folder has its target edited (**card → folder only, not vice versa**); locked even at 0 items. **Deleting a custom folder** removes custom-folder membership from affected items only. An empty folder accepts any item as its first.
...
- **Unsorted resolve (once per card, Unsorted-only UI):** while `sourceLanguage` is **`null`** and resolve not yet used, the user may **Move to language folder** (pick language → sets `sourceLanguage`, moves language folder) **or Move to custom folder** (pick folder → files item; if folder's source is set, item adopts it and moves language folder; if folder empty, user picks source language as part of the action). **At most once per card** — then normal custom-folder re-file only (when source is non-null). These move actions are **not shown on non-Unsorted cards**.
...
**Resolved — custom folder assignment:** **manual only** in v1 — no AI-suggested folder. **Default custom folder** (last used or manually chosen, prefilled on in-app add, auto-applied on widget/Share when language matches) is sticky convenience, not a separate assignment rule.
