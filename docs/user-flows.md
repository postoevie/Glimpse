...
**Entry points:** Quick note widget · Share Extension · in-app "add" on **root** or **folder detail** — widget/Share have no folder picker; they use **default custom folder on save** only when detection is **non-null** and matches that folder's `sourceLanguage` (otherwise language folder only).

1. User provides text (word or phrase) — typed, pasted, or shared from another app.
2. Optional, never required: type a translation; **source language** (auto-detected, **`null` if detection fails and user skips manual pick**, or manual when UI allows — **except** while a custom folder is selected); **target language** (default: source when non-null); on **root** (and Unsorted) Add, **custom folder always prefilled with default** when one exists (forces pending source) — user can change or clear. **Add from language-folder detail:** prefill default only when it matches that language. Picker offers **all** custom folders; selecting one **forces** pending source to that folder's code. **Selecting a folder sets pending source to `CustomFolder.sourceLanguage` (not editable / not clearable to null while selected) and may prefill target — target can still change before save.** Clearing the custom-folder selection restores normal source rules. **Add from folder detail:** language folder or custom → source (and default target) from that folder, source locked; **Unsorted** → same as root (including always-prefill default). Saving with a folder updates the default.
3. On save:
   ...
   - If a custom folder remains selected, item source is that folder's code (non-null) and the item is filed there; folder's `targetLanguage` updates to this item's target when present. Translation is **never auto-generated** — blank unless typed in step 2; generate later in Flow 4.
...
2. User creates a custom folder with a **required name and source language**. Empty folders (0 items) are valid; folder `sourceLanguage` is set at create and **immutable**. **`targetLanguage` on the folder is optional** — set/updated when cards with a target are added or when any card's target is edited. **Only items whose `sourceLanguage` matches** the folder may join. Capture picker offers all custom folders (selection forces pending source). Rename or delete anytime; `sourceLanguage` cannot change — create a new folder for a different source language.
3. **Normal re-file** (non-Unsorted cards): user manually files an item into a custom folder whose `sourceLanguage` matches the item, or removes it — **at most one custom folder per item**, manual only. **Re-file into a folder updates the default custom folder.** Does **not** by itself change item `sourceLanguage` or language folder (and does not change folder `sourceLanguage`). Unsorted items cannot use this path.
4. **Unsorted resolve** (Unsorted / null-source cards): **set source language** — pick language → `sourceLanguage` set → item leaves Unsorted. **Or Move to custom folder** — pick folder → item filed in; item **always adopts** that folder’s required `sourceLanguage` (and moves language folder). **Does not change item `targetLanguage`.** After source is non-null, normal re-file applies.
5. Language folder follows **`sourceLanguage`** whenever it changes. **If the user changes source and a custom folder is selected whose source no longer matches, clear custom-folder membership.**
...
2. Find method:
   - **Browse a folder** — tap a language or custom folder, see its items. From folder detail, **Add** opens Flow 1 (same capture sheet; source/target prefilled from the open folder when it has a `sourceLanguage` — language folder or custom; Unsorted → same as root — see Flow 1).
...
1. Card shows: word/phrase, translation, example, **source language** (editable — may show as unknown if `null`), **target language** (editable), and folder(s). **Unsorted cards (`sourceLanguage` null):** set source and/or **Move to custom folder** (Path B adopts folder source). **Non-Unsorted cards:** source still editable; normal custom-folder re-file; changing source clears custom membership if it no longer matches.
...
6. User can edit **`text`**, **source language**, and **target language** — target edit updates custom folder's `targetLanguage` if filed in one (**not vice versa**); **`sourceLanguage` does not change** when only `text` is edited; changing source updates the language folder and clears mismatched custom membership.
