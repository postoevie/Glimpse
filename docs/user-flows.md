...
2. Optional, never required: type a translation; **source language** (auto-detected, **`null` if detection fails and user skips manual pick**, or manual when UI allows — **except** while a custom folder is selected); **target language** (default: source when non-null); **custom folder prefilled with default** only when that default's `sourceLanguage` matches pending non-null source — user can change or clear. Picker offers **all** custom folders; selecting one **forces** pending source to that folder's code. Pending `null` never auto-prefills a default. **Selecting a folder sets pending source to `CustomFolder.sourceLanguage` (not editable / not clearable to null while selected) and may prefill target — target can still change before save.** Clearing the custom-folder selection restores normal source rules. **Add from folder detail:** language folder or custom → source (and default target) from that folder, source locked; **Unsorted** → same detection/manual as root. Saving with a folder updates the default.
3. On save:
...
3. **Normal re-file** (non-Unsorted cards): user manually files an item into a custom folder whose `sourceLanguage` matches the item, or removes it — **at most one custom folder per item**, manual only. **Re-file into a folder updates the default custom folder.** Does **not** change item `sourceLanguage` or language folder (and does not change folder `sourceLanguage`). Unsorted items cannot use this path.
4. **Unsorted resolve** (Unsorted cards only, **once per item**): **Move to language folder** — pick language → `sourceLanguage` set → item leaves Unsorted. **Or Move to custom folder** — pick folder → item filed in; item **always adopts** that folder’s required `sourceLanguage` (and moves language folder). **Does not change item `targetLanguage`.** After resolve, item source is non-null on the custom path; normal re-file applies; Unsorted-only actions hidden.
