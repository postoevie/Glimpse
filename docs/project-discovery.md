...
| Organize              | An item can belong to **at most two folders**: it always lives in its **built-in per-language folder** — automatic, follows `sourceLanguage` — plus **one optional custom folder** (manual only in v1). Item **source language is editable** after save; if a custom folder is selected and the new source does not match it, **clear custom membership**. Custom folders: **name + `sourceLanguage` required at create** (immutable source code; never null/empty/unlocked); **`targetLanguage` optional**, syncs from card add/edit, prefills new captures when a folder is selected; while folder selected at capture, item source is that folder's code (not editable); may contain 0 items; only items matching folder `sourceLanguage` can join |
...
**Every card has its own `targetLanguage`, user-selectable at creation and editable on the card** (default: source language). Selecting a custom folder at capture **sets** source from that folder (not editable while selected) and **prefills** target from that folder if set — overwriting any prior pick; user can still change target before save. Editing a card's target **updates** its custom folder's `targetLanguage`. Folder target does **not** retroactively change other cards.
